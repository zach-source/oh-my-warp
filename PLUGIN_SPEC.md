...
| **3. JS plugin host** (`app/src/plugin/`) | in-app behavior | JavaScript (QuickJS) | `~/.warp/plugins/*/main.js` (+ optional `plugin.json`) → subprocess | ✅ **enabled** (M0–M4: `warp.log`, `warp.commands`, `warp.terminal`, `warp.ui` (toast/markdown/palette), `warp.keymap`, `warp.ai`, `warp.fs`, `warp.process`, `warp.network`, `warp.plugin`) |
