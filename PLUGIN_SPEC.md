...
- ✅ **Patch 0028:** `warp.ui.showPalette(title, items)` — a plugin-driven picker; each item's `onSelect` callback is invoked in-process via `CallJsFunctionService` on selection (`workspace/view/plugin_palette_modal.rs`). Uses fire-and-forget per-item callbacks rather than a returned Promise, avoiding cross-process async/job-pumping.
