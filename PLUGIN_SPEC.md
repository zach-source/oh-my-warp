...
- ⏳ **Remaining:** declarative `contributes.commands` (palette listing + lazy activation) and `themes`/`workflows` (route into tier-1 loaders); `warp.ui.showMarkdown`/`showPalette`; interactive first-use **consent prompts** for the high-sensitivity capabilities (today they are gated by manifest declaration and shown in Settings → Plugins).
