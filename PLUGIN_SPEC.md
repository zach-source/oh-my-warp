...
| **3. JS plugin host** (`app/src/plugin/`) | in-app behavior | JavaScript (QuickJS) | `~/.warp/plugins/*/main.js` (+ optional `plugin.json`) → subprocess | ✅ **enabled** (M0–M4: `warp.log`, `warp.commands`, `warp.terminal`, `warp.ui` (toast/markdown/palette), `warp.keymap`, `warp.ai`, `warp.fs`, `warp.process`, `warp.network`, `warp.plugin`, `warp.prompt`) |
...
- ✅ **Patch 0026:** high-sensitivity capabilities — `warp.fs` (`readFile`/`readDir` gated by `fs:read`, `writeFile` gated by `fs:write`), `warp.process` (`exec`, gated by `process`), and `warp.network` (`fetch`, gated by `network`). Each namespace is exposed only when the manifest grants the permission; calls run host-side (sync `std::fs`/`std::process`; blocking `reqwest` for network), throw catchable JS exceptions on error, and are logged for auditability (`js_api/mod.rs`).
- ✅ **Patch 0027:** `warp.ui.showMarkdown(title, markdown)` — renders plugin-supplied markdown in a workspace modal (`parse_markdown` + `FormattedTextElement`), opened via a `PluginHostEvent` the `Workspace` subscribes to (`workspace/view/plugin_markdown_modal.rs`).
- ✅ **Patch 0028:** `warp.ui.showPalette(title, items)` — a plugin-driven picker; each item is `{ label, command }` naming a command registered via `warp.commands.register`. On selection the picker emits the command id and the `Workspace` runs it as a fresh `RunPluginCommand` (`workspace/view/plugin_palette_modal.rs`). Items reference commands (not inline callbacks) so `showPalette` never re-enters `plugin.get_mut()` from inside the calling callback — doing so panics the host (`BorrowMutError` → `JS_FreeRuntime` abort).
- ✅ **Patch 0040:** `warp.prompt.set([{ text, side?, tooltip? }])` / `warp.prompt.clear()` (capability: `ui`) — plugin-contributed prompt segments rendered as native chips after the built-in ones. The host can't render synchronously, so it's push: the plugin computes segment text and pushes it (e.g. from `terminal.onCommandFinished`); a singleton `PluginPromptModel` (`context_chips/plugin_prompt.rs`) caches it keyed by plugin id, and every `PromptDisplay` observes it and re-renders (`context_chips/display.rs`, reusing `render_udi_chip`). `side` groups segments left-then-right; true right-edge alignment is a follow-up (the modern UDI prompt has no rprompt region). Demoed by the `claude-usage` plugin's live "today" cost chip.
- ✅ **Patch 0041:** plugin prompt segments render in **both** the terminal prompt and the agent input footer. Extracted `render_plugin_chips_for_side(side, app, appearance)` into `context_chips/plugin_prompt.rs`; both `PromptDisplay::render` and `AgentInputFooter::render` call it. No JS API change.
- ✅ **Patch 0042:** richer `warp.ui.showPalette` — items grow optional `description` (dimmed inline subtitle), `icon` (sigil/emoji rendered in a fixed-width left column), and `kbd` (monospace keystroke chip on the right). The picker now has a search input at the top with built-in fuzzy matching against label + description, keyboard nav (up/down/enter), and selected/hover row backgrounds — the same look as the native command palette. Item schema: `{ label, command, description?, icon?, kbd? }`. See `workspace/view/plugin_palette_modal.rs`.
- ✅ **Patch 0043:** polish for `warp.ui.showMarkdown` — `ClippedScrollable` wraps the body (long docs stay in the frame, scroll position resets on a new open), inline `` `code` `` picks up themed background+color, hyperlinks render in the accent color, and the modal grows to 640×560 with 20×28 body padding so headings/lists breathe. No JS API change.
- ✅ **Patch 0044:** `warp.prompt` segments grow `kind` (`info | success | warn | error | accent` → ANSI theme color) and `icon` (sigil/emoji prefixed to the chip text). Missing `kind` defaults to `info` (the original ansi-blue), so segments pushed before this field existed render unchanged. Item schema: `{ text, side?, tooltip?, kind?, icon? }`. See `context_chips/plugin_prompt.rs`.
- ✅ **Patch 0045:** `warp.ui.setStatusItem(id, item)` — plugin-contributed tab-bar pills for live status (CI state, queue depth, billing). Item shape `{ text, kind?, tooltip?, command? }`; `command` makes the pill clickable and dispatches `WorkspaceAction::RunPluginCommand(id)` — same routing palette items use. Identity is `(plugin_id, item_id)`, so one plugin can publish several pills (e.g. branch + CI + queue) and update each independently. `null` (or empty `text`) removes a pill. Singleton model: `workspace/plugin_status_items.rs`; rendered in `render_tab_bar_contents` right of the leader indicator. Capability: `ui`.
- ⏳ **Remaining (smaller follow-ups):** declarative `contributes.commands` (palette listing + lazy activation) and `themes`/`workflows` (route into tier-1 loaders); interactive first-use **consent prompts** for the high-sensitivity capabilities (today gated by manifest declaration and shown in Settings → Plugins); `warp.ui.theme` read-only token snapshot (needs an app→host push at bootstrap, with a refresh on theme change) and `warp.terminal.annotateBlock` (block decoration — needs to thread into the block view chrome). These are tracked but deliberately deferred to a follow-up pass.
