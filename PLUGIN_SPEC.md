...
- ⏳ **Remaining (smaller follow-ups):** declarative `contributes.commands` (palette listing + lazy activation) and `themes`/`workflows` (route into tier-1 loaders); interactive first-use **consent prompts** for the high-sensitivity capabilities (today gated by manifest declaration and shown in Settings → Plugins).
